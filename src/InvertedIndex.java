...
public class lab1 {
	...
	public void addEntry(String word, int x, int y) throws IOException {
		...
		// ADD YOUR CODES HERE
		// Test if a term has already existed in the inverted index file. If so,
		// stop insertion and return directly.
		// If not, insert to the inverted file;
		// If there is no the following "if" block, the posting lists will
		// accumulate after several time's running the code.
		if (hashtable.get(word) != null
				&& ((String) hashtable.get(word)).contains("doc" + x + " " + y)) {
			return;
		}
